...
export default function ProductForm({ product, onSave, onCancel }: ProductFormProps) {
  ...
  const inputClass =
    'w-full bg-[#F7F7F5] border border-[#E8E2D9] px-3 py-2.5 font-sans text-xs text-[#1A1714] placeholder-[#333] focus:border-[#8B7355] focus:outline-none';
  const labelClass = 'block font-sans text-xs text-[#6B6459] uppercase tracking-[0.15em] mb-1';
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-6">
      ...
      <div>
        ...
        {errors.category_id && <p className="font-sans text-xs text-[#8B7355] mt-1">{errors.category_id.message}</p>}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="flex items-center gap-4 mt-5">
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            <span className="font-sans text-xs text-[#6B6459]">Active</span>
          </label>
          <label className="flex items-center gap-2 cursor-pointer">
            <input type="checkbox" {...register('is_featured')} className="sr-only" />
            <span className="font-sans text-xs text-[#6B6459]">Featured</span>
          </label>
        </div>
      </div>
        ...
        <div className="flex gap-2 flex-wrap mb-3">
          {images.map((url, i) => (
            <div key={i} className="relative">
              <img src={url} alt={`Product ${i + 1}`} className="w-16 h-16 object-cover border border-[#E8E2D9]" />
              <button
                type="button"
                onClick={() => setValue('images', images.filter((_, j) => j !== i))}
                className="absolute -top-1 -right-1 w-4 h-4 bg-[#1A1714] text-black text-xs flex items-center justify-center"
              >
                ×
              </button>
            </div>
          ))}
          <label className="w-16 h-16 border border-dashed border-[#E8E2D9] flex flex-col items-center justify-center cursor-pointer hover:border-[#8B7355] transition-colors">
            <Upload size={14} className="text-[#6B6459]" />
            <input type="file" multiple accept="image/*" onChange={handleImageUpload} className="sr-only" />
          </label>
        </div>
        {uploading && <p className="font-sans text-xs text-[#6B6459]">Uploading...</p>}
      </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          ...
          <button
            ...
            className="flex items-center gap-1 font-sans text-xs text-[#8B7355] hover:text-[#1A1714] transition-colors"
          >
        ...
        <div className="space-y-4">
          {fields.map((field, idx) => (
            <div key={field.id} className="border border-[#E8E2D9] p-4 relative">
              <button
                type="button"
                onClick={() => remove(idx)}
                className="absolute top-2 right-2 text-[#6B6459] hover:text-[#8B7355] transition-colors"
              >
                <Trash2 size={13} />
              </button>
