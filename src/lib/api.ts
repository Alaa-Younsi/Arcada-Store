...
const STORAGE_BASE = `${import.meta.env.VITE_SUPABASE_URL}/storage/v1/object/public/product-images/products`;

/** Converts a bare filename (ARC-SIL-001.jpg) to a full Supabase Storage URL.
...
export const api = {
  ...
  products: {
    getAll: async (params?: {
      ...
      if (error) throw new Error(error.message);
      return { data: ((data as Product[]) ?? []).map(resolveProductImages), count: count ?? 0, page, limit };
    },
    ...
    getFeatured: async (): Promise<Product[]> => {
      ...
      if (featured.length >= 4) return featured.slice(0, 4).map(resolveProductImages);
      ...
      return [...featured, ...supplemental].slice(0, 4).map(resolveProductImages);
    },
    ...
    getByCategory: async (slug: string): Promise<Product[]> => {
      ...
      const { data, error } = await supabase
        ...
        .eq('is_active', true);
      if (error) throw new Error(error.message);
      return ((data as Product[]) ?? []).map(resolveProductImages);
    },
    ...
    getBySlug: async (slug: string): Promise<Product> => {
      const { data, error } = await supabase
        ...
        .single();
      if (error) throw new Error(error.message);
      return resolveProductImages(data as Product);
    },
  },
